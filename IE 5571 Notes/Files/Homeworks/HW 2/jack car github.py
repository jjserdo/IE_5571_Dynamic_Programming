...
class Params:
    def __init__(self):
        ...
        #self.problem_types = ['original_problem']
        self.problem_types = ['modified_problem']
...
class PolicyIteration:
    ...
    def print_pis(self):
        ...
        for idx, pi in enumerate(self.pis):
            ...
            ax.imshow(pi, origin="lower", interpolation='none', vmin=-self.params.max_move, vmax=self.params.max_move)
            ax.set_title(f"Jack's Special Problem Iteration: {idx}")
            fig.tight_layout()
            ax.set_xlabel("Cars at Location 2")
            ax.set_ylabel("Cars at Location 1")
            ax.set_xticks(np.arange(0, self.params.max_car+1, 5))
            ax.set_yticks(np.arange(0, self.params.max_car+1, 5))
            for yy in range(pi.shape[1]):
                for xx in range(pi.shape[0]):
                    text = ax.text(xx, yy, pi[yy, xx], ha="center", va="center", color="w")
    ...
    def print_V(self):
        ...
        X, Y = np.meshgrid(range( self.params.max_car+1), range(self.params.max_car+1))
        ax.plot_wireframe(X, Y, self.V, rstride=1, cstride=1)
        ax.set_xlabel("Cars at Location 2")
        ax.set_ylabel("Cars at Location 1")
        ax.set_zlabel("V", rotation=0)
        ax.set_title("Value Function for Jack's Special Problem")
        ax.set_xticks(np.arange(0, self.params.max_car+1, 5))
        ax.set_yticks(np.arange(0, self.params.max_car+1, 5))
